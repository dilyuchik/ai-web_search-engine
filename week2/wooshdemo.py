...
#
# The "stored" attribute (here, title and content) is used for all parts that we want to be able to fully retrieve from the index
#
schema = Schema(title=TEXT(stored=True), content=TEXT)

# Create an index in the directory indexdr (the directory must already exist!)
ix = create_in("indexdir", schema)
writer = ix.writer() # write things to index

# now let's add some texts (=documents) retrieved from crawled websites
...
# build searcher method, searching for documents in compliance with search entries, and ranking search results
